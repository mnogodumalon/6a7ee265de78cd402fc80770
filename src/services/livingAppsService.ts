...
import type { Uebungen, Trainingslog, CreateUebungen, CreateTrainingslog } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getUebungen(): Promise<Uebungen[]> {
    ...
    return enrichLookupFields(records, 'uebungen');
  }
  static async getUebungenEntry(id: string): Promise<Uebungen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Uebungen;
    return enrichLookupFields([record], 'uebungen')[0];
  }
  static async createUebungenEntry(fields: CreateUebungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.UEBUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'uebungen') });
    return { ...data, record_id: data.id };
  }
  static async updateUebungenEntry(id: string, fields: Partial<CreateUebungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.UEBUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'uebungen') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- TRAININGSLOG ---
  static async getTrainingslog(): Promise<Trainingslog[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TRAININGSLOG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Trainingslog[];
    return enrichLookupFields(records, 'trainingslog');
  }
  static async getTrainingslogEntry(id: string): Promise<Trainingslog | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TRAININGSLOG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Trainingslog;
    return enrichLookupFields([record], 'trainingslog')[0];
  }
  static async createTrainingslogEntry(fields: CreateTrainingslog): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.TRAININGSLOG}/records`, { fields: cleanFieldsForApi(fields as any, 'trainingslog') });
    return { ...data, record_id: data.id };
  }
  static async updateTrainingslogEntry(id: string, fields: Partial<CreateTrainingslog>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.TRAININGSLOG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'trainingslog') });
    return { ...data, record_id: data.id };
  }
  static async deleteTrainingslogEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TRAININGSLOG}/records/${id}`);
  }
