...
import { UebungenViewDialog } from '@/components/dialogs/UebungenViewDialog';
import { TrainingslogDialog } from '@/components/dialogs/TrainingslogDialog';
import { TrainingslogViewDialog } from '@/components/dialogs/TrainingslogViewDialog';
...
const UEBUNGEN_FIELDS = [
  { key: 'name', label: 'Übungsname', type: 'string/text' },
  { key: 'muskelgruppe', label: 'Muskelgruppe', type: 'lookup/select', options: [{ key: 'brust', label: 'Brust' }, { key: 'ruecken', label: 'Rücken' }, { key: 'schultern', label: 'Schultern' }, { key: 'arme', label: 'Arme' }, { key: 'beine', label: 'Beine' }, { key: 'bauch', label: 'Bauch' }, { key: 'ganzkoerper', label: 'Ganzkörper' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const TRAININGSLOG_FIELDS = [
  { key: 'datum', label: 'Datum & Uhrzeit', type: 'date/datetimeminute' },
  { key: 'uebung', label: 'Übung', type: 'applookup/select', targetEntity: 'uebungen', targetAppId: 'UEBUNGEN', displayField: 'name' },
  { key: 'satz_nummer', label: 'Satznummer', type: 'number' },
  { key: 'gewicht_kg', label: 'Gewicht (kg)', type: 'number' },
  { key: 'wiederholungen', label: 'Wiederholungen', type: 'number' },
  { key: 'notiz', label: 'Notiz', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'uebungen', pascal: 'Uebungen' },
  { key: 'trainingslog', pascal: 'Trainingslog' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('uebungen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'uebungen': new Set(),
    'trainingslog': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'uebungen': return (data as any).uebungen as Uebungen[] ?? [];
      case 'trainingslog': return (data as any).trainingslog as Trainingslog[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'trainingslog':
        lists.uebungenList = (data as any).uebungen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'trainingslog' && fieldKey === 'uebung') {
      const match = (lists.uebungenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'uebungen': return UEBUNGEN_FIELDS as any[];
        case 'trainingslog': return TRAININGSLOG_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'uebungen': return {
        create: (fields: any) => LivingAppsService.createUebungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateUebungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteUebungenEntry(id),
      };
      case 'trainingslog': return {
        create: (fields: any) => LivingAppsService.createTrainingslogEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTrainingslogEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTrainingslogEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'uebungen' || dialogState?.entity === 'uebungen') && (
        <UebungenDialog
          open={createEntity === 'uebungen' || dialogState?.entity === 'uebungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'uebungen' ? handleUpdate : (fields: any) => handleCreate('uebungen', fields)}
          defaultValues={dialogState?.entity === 'uebungen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Uebungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Uebungen']}
        />
      )}
      {(createEntity === 'trainingslog' || dialogState?.entity === 'trainingslog') && (
        <TrainingslogDialog
          open={createEntity === 'trainingslog' || dialogState?.entity === 'trainingslog'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'trainingslog' ? handleUpdate : (fields: any) => handleCreate('trainingslog', fields)}
          defaultValues={dialogState?.entity === 'trainingslog' ? dialogState.record?.fields : undefined}
          uebungenList={(data as any).uebungen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Trainingslog']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Trainingslog']}
        />
      )}
      {viewState?.entity === 'uebungen' && (
        <UebungenViewDialog
          open={viewState?.entity === 'uebungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'uebungen', record: r }); }}
        />
      )}
      {viewState?.entity === 'trainingslog' && (
        <TrainingslogViewDialog
          open={viewState?.entity === 'trainingslog'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'trainingslog', record: r }); }}
          uebungenList={(data as any).uebungen ?? []}
        />
      )}
